...
test(
  ...
  async () => {
    ...
    await expect(provider.call('cfx_epochNumber')).rejects.toThrow(
      'BaseProvider.request not implement'
    )
...
test('Invalid provider', () => {
  expect(() => providerFactory({ url: 'xxx' })).toThrow(
...
test.skip(
  ...
  async () => {
    ...
    const array = await provider.batch([
